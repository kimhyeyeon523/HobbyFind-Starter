...
> = {
  exercise: {
    label: '운동형',
    className:
      'border-brand/30 bg-brand/10 text-brand ring-1 ring-inset ring-brand/20',
  },
  intellectual: {
    label: '지능형',
    className:
      'border-blue-200 bg-blue-50 text-blue-900 ring-1 ring-inset ring-blue-100',
  },
  art: {
    label: '예술형',
    className:
      'border-violet-200 bg-violet-50 text-violet-900 ring-1 ring-inset ring-violet-100',
  },
};
    ...
    <article className="group overflow-hidden rounded-xl border border-neutral-200 bg-white shadow-sm transition-shadow hover:shadow-md">
      ...
      <div className="space-y-2 p-4">
        ...
        <Link
          ...
          className={cn(
            'inline-flex rounded-full border px-2.5 py-0.5 text-xs font-semibold transition-opacity hover:opacity-90',
            tag.className,
          )}
        >
          {tag.label}
